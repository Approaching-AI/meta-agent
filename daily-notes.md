...
This file describes `meta-agent`'s own internal session log.
...
- **Append-only by default.** Meta-log entries are records of what happened. Avoid modifying past entries.
...
每次 session 的 meta-log 记录应标注操作者（即当时与 agent 协作的人）。在 session 对应的 section 开头加一行：
...
What goes into `meta-log/` is decided by the agent and its human collaborator together. Typical triggers include:
