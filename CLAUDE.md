...
- `daily-notes.md` — how to keep daily logs (stored in `meta-log/`)
...
1. **总结写入 daily notes** — 记录本次 session 做了什么、遇到什么问题、下一步是什么。Append 到当天的 `meta-log/` 文件。记录开头标注 `operator: <name>`（与 agent 协作的人）。如果不知道操作者是谁，主动询问，不能用 git config 或其他环境信息代替确认。
